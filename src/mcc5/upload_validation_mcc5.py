"""
Validates a real, arbitrary uploaded CSV before it's handed to the pipeline.
This is deliberately separate from pipeline_mcc5.check_motor():
A file either passes these checks and gets a validated, schema-normalized DataFrame,
or it's rejected with a specific, user-facing reason.
The model is never run on something that hasn't passed this gate.

What's actually checked, and why:

1. Column shape. Every model/feature in this project only ever reads 4 columns:
   current_a, current_b, current_c, torque. "time", "speed", and the 3 vibration channels
   are in the raw dataset but genuinely unused anywhere. A real upload without vibration sensors
   should not be rejected over columns nothing reads. Two shapes are accepted:
...
2. Sample rate. Every frequency-domain feature is built around FS=12800Hz.
   Getting this wrong doesn't degrade results, it silently produces confidently wrong ones.
   Current/torque values alone can't reveal a file's true sample rate there are exactly two
...
3. Minimum length. Checked directly by truncating real recordings: shaft_hz and condition
   ...
   (~0.13Hz) never stabilizes until close to the full ~90s recording. 60s is the hard
...
FULL_REFERENCE_DURATION_SEC = 90.0 
SAMPLE_RATE_TOLERANCE = 0.01  # a real recording's rate should match almost exactly
...
class UploadValidationError(Exception):
    """Raised with a specific, user-facing reason.
    Meant to be caught by the app layer and shown directly, not a generic failure."""
